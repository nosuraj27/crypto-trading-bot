...
class BinanceService {
    ...
    /**
     * Create a withdrawal
     * @param {string} coin - The coin to withdraw
     * @param {string} address - The address to withdraw to
     * @param {string} network - The network to use for withdrawal (e.g., 'BSC', 'ETH', 'BTC')
     * @param {number} amount - The amount to withdraw
     * @param {string} [memo] - Optional memo/tag for some coins
     * @returns {Promise<Object>} - Withdrawal result
     */
    async withdraw(params) {
        const { coin, address, network, amount, memo } = params;

        // Check testnet mode
        if (this.testnet) {
            console.warn('⚠️ Withdrawal attempted in testnet mode - this would be skipped in production');
            return {
                success: true,
                message: 'Withdrawal simulated in testnet mode',
                coin,
                address,
                amount,
                id: 'TESTNET-' + Date.now()
        ...
        return this.signedRequest('/sapi/v1/capital/withdraw/apply', withdrawParams, 'POST');
    }

    /**
     * Get deposit address for a coin
     * @param {string} coin - The coin to get address for
     * @param {string} network - The network to use
     * @returns {Promise<Object>} - Deposit address info
     */
    async getDepositAddress(coin, network) {
        return this.signedRequest('/sapi/v1/capital/deposit/address', {
            coin: coin.toUpperCase(),
            network: network.toUpperCase()
        });
    }
