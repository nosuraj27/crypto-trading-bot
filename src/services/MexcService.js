...
class MexcService {
    constructor(testnet = false) {
        ...
        // MEXC has testnet/sandbox environment
        this.baseURL = testnet ? "https://api.mexc.com" : "https://api.mexc.com"; // Same URL for now
        this.apiKey = process.env.MEXC_API_KEY;
        this.apiSecret = process.env.MEXC_API_SECRET;

        if (!this.apiKey || !this.apiSecret) {
            console.warn('MEXC API credentials not found in environment variables');
        }
        ...
        console.log(`🔧 MexcService initialized: ${testnet ? 'TESTNET' : 'MAINNET'} mode`);
    }
    ...
    async apiHandler(requestFn, errorMessage) {
        try {
            ...
            console.error(`❌ MEXC API Error: ${errorMsg}`);
    ...
    async testConnectivity() {
        try {
            ...
            console.log(`✅ MEXC ${this.testnet ? 'Testnet' : 'Mainnet'} connectivity OK`);
            return true;
        } catch (error) {
            console.error(`❌ MEXC ${this.testnet ? 'Testnet' : 'Mainnet'} connectivity failed:`, error.message);
            return false;
        }
    }
    ...
    async testAuthentication() {
        try {
            ...
            console.log(`✅ MEXC ${this.testnet ? 'Testnet' : 'Mainnet'} authentication OK`);
            return true;
        } catch (error) {
            console.error(`❌ MEXC ${this.testnet ? 'Testnet' : 'Mainnet'} authentication failed:`, error.message);
            return false;
        }
    }
}
